...
if type "lsd" >/dev/null 2>&1; then
	alias oldls="/bin/ls"
	alias ls='lsd --git'
	alias la='lsd --all --prompt --size bytes --git --group-directories-first'
	alias ll="lsd --long --prompt --size bytes --git --group-directories-first --date '+%Y-%m-%dT%H:%M:%S'"
	alias lla="lsd --long --all --prompt --size bytes --git --group-directories-first --date '+%Y-%m-%dT%H:%M:%S'"
	alias lt="lsd --tree"
...
if type "fd" >/dev/null 2>&1; then
	...
	alias find='fd'
	alias pcd='cd $(fd -t d | peco --prompt="Change Directory to ... >")'
else
	alias pcd='cd $(find . --maxdepth 1 --type d | peco --prompt="cd to...>")'
fi
...
if type "docker" >/dev/null 2>&1; then
	...
	alias de='docker exec -it $(docker ps | peco --prompt="Choose Container >" | cut -d " " -f 1) /bin/bash'

	# 選択したコンテナを削除する
	alias drm='docker rm $(docker ps | peco --prompt="Choose Delete Container >" | cut -d " " -f 1)'
	# 選択したdocker imageを削除する
	alias drmi='docker rm $(docker images -a | peco --prompt="Choose Delete Image >" | cut -d " " -f 8)'
