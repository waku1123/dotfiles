...
return {
  ...
  keys = {
    ...
    { "<M-r><M-t>f", ":lua require('neotest').run.run(vim.fn.expand('%'))<CR>", mode = "n", desc = "現在のファイルのテストを実行する" },
    { "<M-r><M-t>v", ":lua require('neotest').output.open()<CR>", mode = "n", desc = "テストの結果を表示する(float window)" },
